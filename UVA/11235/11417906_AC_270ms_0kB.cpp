...
void build(int node , int st ,int en)
{
    ...
    if(lvalue[2*node+1]==rvalue[2*node]){
        ...
        if(tree[node]<rfre[2*node]+lfre[2*node+1])ans[node] = lvalue[2*node];
...
data query(int node ,int st ,int en , int l , int r)
{
    ...
    if(p.value==p1.value)p.ans = p1.ans;
    else p.ans = p2.ans;

    if(p2.lvalue==p1.rvalue){

        if(p1.rfre+p2.lfre>p.value)p.ans = p2.value;
